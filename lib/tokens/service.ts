...
export class TokenService {
  ...
  async execute(proxy: ProviderProxy, adapter: ProviderAdapter, input: unknown, signal: AbortSignal, agent: string, systemPrompt: string, messages?: RequestOptions['messages'], bypassCache = false) {
    ...
    if (rows.some(row => row.used + row.reserved >= row.limit)) { this.pause(agent); this.refuse(agent, 'Token budget exhausted.'); }
    ...
    try {
      ...
      if (![usage.prompt, usage.completion, usage.total].every(validLimit) || usage.total !== usage.prompt + usage.completion) this.refuse(agent, 'Provider usage invalid.');
